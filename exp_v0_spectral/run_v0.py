...
from sde_rf_wan.spectral import (
    IdentitySchedule,
    LinearCutoffSchedule,
    SPDCutoffSchedule,
    HighPassReverseSchedule,
)
from sde_rf_wan.spectral_codebook import ThreeBandStepSchedule
...
def main():
    ...
    # ----- V0 spectral mask arguments -----
    parser.add_argument("--spectral_schedule",
                        choices=["identity", "linear", "spd", "highpass_reverse"],
                        default="identity",
                        help="V0 spectral mask schedule (identity = baseline GVCC)")
    parser.add_argument("--spectral_low_c", type=float, default=0.2,
                        help="Linear schedule: cutoff at t=1 (high noise)")
    parser.add_argument("--spectral_high_c", type=float, default=1.0,
                        help="Linear schedule: cutoff at t=0 (clean)")
    parser.add_argument("--spectral_beta", type=float, default=1.988,
                        help="SPD schedule: power-law β (default = our Wan latent fit)")
    parser.add_argument("--spectral_intercept", type=float, default=-4.074,
                        help="SPD schedule: power-law intercept log c")
    parser.add_argument("--spectral_delta", type=float, default=0.01,
                        help="SPD schedule: δ tolerance (smaller = more aggressive low-pass)")
    parser.add_argument("--spectral_soft_taper", action="store_true",
                        help="Use cosine taper at the mask cutoff (anti-ringing)")
    # ----- V2-a spectral codebook scheduling -----
    parser.add_argument("--codebook_mode",
                        choices=["full", "v2a_masked_random", "v2a_spectrum_preserving",
                                 "v2a_gaussian_control", "v2a_selection_only"],
                        default="full",
                        help="full = baseline; v2a_masked_random = band-limited (failed control); "
                             "v2a_spectrum_preserving = M_B·z_sel + (1-M_B)·eps; "
                             "v2a_gaussian_control = M_B·eps1 + (1-M_B)·eps2 (no codebook); "
                             "v2a_selection_only = band-aware score, full-atom injection")
    parser.add_argument("--v2a_low_until", type=int, default=6,
                        help="V2-a: SDE steps before this use low band only")
    parser.add_argument("--v2a_mid_until", type=int, default=12,
                        help="V2-a: SDE steps before this use low+mid band")
    parser.add_argument("--v2a_low_cutoff", type=float, default=1.0 / 3.0,
                        help="V2-a: normalized radial cutoff for low band")
    parser.add_argument("--v2a_mid_cutoff", type=float, default=2.0 / 3.0,
                        help="V2-a: normalized radial cutoff for low+mid band")
    parser.add_argument("--v2a_soft_taper", action="store_true",
                        help="V2-a: use soft taper at active band boundary")
    parser.add_argument("--vectorized_atom_gen", action="store_true",
                        help="Use vectorized torch.randn(count, D) for atom batch "
                             "generation (saves kernel-launch overhead, ~2× encode "
                             "speedup on per-frame baseline). Encoder + decoder "
                             "must use the same setting; bitstream values differ "
                             "from non-vectorized mode but quality is identical.")
    parser.add_argument("--skip_model_alternate", action="store_true",
                        help="Legacy alias: equivalent to --model_call_skip u_cache --model_call_period 2.")
    parser.add_argument("--model_call_skip", choices=["none", "u_cache", "x0_cache"],
                        default="none",
                        help="Model-call skipping mode: none / reuse cached u_t / "
                             "reuse cached x0_hat (then compute u_t = (x_t - x0)/t).")
    parser.add_argument("--model_call_period", type=int, default=2,
                        help="Skip when step_idx %% period != 0 (period=2 = every other, "
                             "period=3 = every 3rd, etc).")
    parser.add_argument("--model_call_skip_until", type=int, default=-1,
                        help="Only skip in steps < this index (-1 = all SDE steps).")
    args = parser.parse_args()

    # Build the spectral schedule from args
    if args.spectral_schedule == "identity":
        spectral_schedule = IdentitySchedule()
    elif args.spectral_schedule == "linear":
        spectral_schedule = LinearCutoffSchedule(
            c_high_t=args.spectral_low_c, c_low_t=args.spectral_high_c)
    elif args.spectral_schedule == "spd":
        spectral_schedule = SPDCutoffSchedule(
            beta=args.spectral_beta,
            intercept_log_c=args.spectral_intercept,
            delta=args.spectral_delta)
    elif args.spectral_schedule == "highpass_reverse":
        spectral_schedule = HighPassReverseSchedule(
            c_high_t=args.spectral_low_c, c_low_t=args.spectral_high_c)
    else:
        spectral_schedule = IdentitySchedule()

    spectral_codebook_schedule = None
    if args.codebook_mode in ("v2a_masked_random", "v2a_spectrum_preserving",
                              "v2a_gaussian_control", "v2a_selection_only"):
        spectral_codebook_schedule = ThreeBandStepSchedule(
            low_until=args.v2a_low_until,
            mid_until=args.v2a_mid_until,
            low_cutoff=args.v2a_low_cutoff,
            mid_cutoff=args.v2a_mid_cutoff,
            soft=args.v2a_soft_taper,
        )
        if args.spectral_schedule != "identity":
            print(
                "Warning: --spectral_schedule is ignored when "
                "--codebook_mode uses V2-a spectral codebook selection."
            )
    ...
    model_name = Path(args.wan_ckpt).name
    ...
    print(f"  Codebook mode: {args.codebook_mode}")
    if spectral_codebook_schedule is not None:
        print(f"  V2-a schedule: {spectral_codebook_schedule}")
    ...
    for seq_idx, (seq_name, seq_path) in enumerate(all_seqs):
        ...
        for g in range(len(gops_gt)):
            ...
            pipe = TurboDDCMWanPipeline(
                ...
                spectral_soft_taper=args.spectral_soft_taper,
                spectral_codebook_schedule=spectral_codebook_schedule,
                spectral_codebook_preserve_spectrum=(
                    args.codebook_mode == "v2a_spectrum_preserving"
                ),
                spectral_codebook_mode={
                    "full":                       None,
                    "v2a_masked_random":          "band_masked",
                    "v2a_spectrum_preserving":    "spectrum_preserving",
                    "v2a_gaussian_control":       "gaussian_control",
                    "v2a_selection_only":         "selection_only",
                }[args.codebook_mode],
                vectorized_atom_gen=args.vectorized_atom_gen,
                skip_model_alternate=args.skip_model_alternate,
            ...
            result = {
                ...
                "spectral_schedule": repr(spectral_schedule),
                "spectral_codebook_schedule": (
                    repr(spectral_codebook_schedule)
                    if spectral_codebook_schedule is not None else None
                ),
    ...
    summary = {
        ...
        "config": {
            ...
            "v2a_low_until": args.v2a_low_until,
            "v2a_mid_until": args.v2a_mid_until,
            "v2a_low_cutoff": args.v2a_low_cutoff,
            "v2a_mid_cutoff": args.v2a_mid_cutoff,
        },
