...
\emph{codec}: the bitstream specifies the stochastic innovations
$\{\mathbf{z}^*_k\}$, and the decoder must reproduce the same trajectory as
the encoder. Directly reducing $T_\text{sde}$ therefore removes both solver
steps and opportunities to inject codebook residuals. GVCCTurbo instead keeps
the correction grid
\begin{equation}
\mathcal{K}_\text{sde}=\{1,\ldots,T_\text{sde}\}
\end{equation}
fixed, and reduces the cost of evaluating the drift on a subset
$\mathcal{Q}\subseteq\mathcal{K}_\text{sde}$:
\begin{equation}
\tilde{\mathbf{u}}_k =
\begin{cases}
\mathbf{u}_\theta(\mathbf{x}_{t_k},t_k), & k\in\mathcal{Q},\\
\mathrm{Predict}(\mathbf{x}_{t_k},t_k,\mathcal{C}), & k\notin\mathcal{Q},
\end{cases}
\label{eq:predictive-u}
\end{equation}
where $\mathcal{C}$ is a decoder-reproducible cache. Equation~\ref{eq:sde-step}
is then applied at \emph{every} SDE step with the transmitted
$\mathbf{z}^*_k$ unchanged. Multi-resolution coding follows the same
principle spatially: early steps may run on a smaller latent grid, but the
stage target must remain a projection of the final high-resolution latent so
that the lifted trajectory is still a trajectory toward the same
$\mathbf{x}_0$. The rest of this section instantiates these two ways of
reducing generator cost while preserving the codebook correction channel.
...
Equivalently, the low stage is optimized against
$\mathbf{x}_0^{(s),\,\text{naive}}$, but after spectral lift the full-stage
loss is sensitive to
\begin{equation}
\mathcal{T}_L(\bar{\mathbf{x}}_{t_k}^{(s+1)})-\mathcal{T}_L(\mathbf{x}_0),
\label{eq:lifted-low-error}
\end{equation}
not to
$\mathbf{x}^{(s)}_{t_k}-\mathbf{x}_0^{(s),\,\text{naive}}$. Thus a perfect
codebook match in the native low-resolution VAE domain can still become a
biased initialization in the high-resolution codec domain.
...
The transition high band therefore has the additive form
\begin{equation}
\mathbf{x}^{H}_{t_k}
= t_k\boldsymbol{\epsilon}_H + \widehat{\mathbf{r}}_{H},
\qquad
\widehat{\mathbf{r}}_{H}\approx (1-t_k)\mathcal{T}_{H}(\mathbf{x}_0),
\label{eq:dlc1-transition}
\end{equation}
which preserves the Gaussian noise floor while using transmitted bits only
for the target-aware clean contribution.
