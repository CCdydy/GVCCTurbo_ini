...
IMPORTANT: encoder and decoder must use the same atom-generation protocol.
The original sequential mode generates one randn(D) atom at a time and is
batch-size invariant but slow. The vectorized mode generates randn(N, D)
batches and is much faster; it is deterministic as long as both sides use the
same gen_batch and vectorized_atom_gen settings.
"""
...
class TurboPerFrameCodebook:
    ...
    def __init__(
        ...
        # In sequential mode it does not affect atom values. In vectorized mode
        # encoder and decoder must use the same gen_batch.
        if gen_batch <= 0:
    ...
    def _generate_atoms_batch(
        ...
        """Generate `count` atoms.

        Two modes:
        - Sequential (default): one randn(D) call per atom. RNG sequence is
          invariant to gen_batch — atom values stay the same if you change
          batch size. ~30-50us per launch × K atoms → dominates wall time.
        - Vectorized (vectorized_atom_gen=True): one randn(count, D) call per
          batch. F× fewer kernel launches → much faster. Atom values become
          dependent on gen_batch (sequence packed differently), but encoder
          and decoder using the SAME setting still agree.

        Returns: (count, D) tensor of atoms.
        """
        if self.vectorized_atom_gen:
            return torch.randn(count, self.D, generator=gen, device=self.device)
    ...
    def _regenerate_and_combine(
        ...
        Must use the SAME batching scheme as select_atoms so atom values match.
        In vectorized mode, atoms are produced in chunks of gen_batch via a
        single randn(bc, D) call per chunk.
        """
        gen = torch.Generator(device=self.device).manual_seed(seed_sf)
        target = set(indices)
        found: Dict[int, torch.Tensor] = {}
        max_needed = max(indices) + 1

        if self.vectorized_atom_gen:
            # Match select_atoms' batching: randn(bc, D) per chunk
            generated = 0
            while generated < max_needed and len(found) < len(indices):
                bc = min(self.gen_batch, self.K - generated)
                batch = torch.randn(bc, self.D, generator=gen, device=self.device)
                for j in range(bc):
                    gi = generated + j
                    if gi in target:
                        found[gi] = batch[j].clone()
