...
class WanFLF2VWrapper:
    ...
    def load(self, device: str = "cuda", dtype: torch.dtype = torch.bfloat16,
        ...
        fp8=True: quantize DiT to torch.float8_e4m3fn (weight-only); matmul
        still runs in `dtype` via VideoX-Fun's wrapper.
        """
        ...
        if fp8:
            import time as _t
            _VFUN = "/home/rog/Desktop/gvcc_turbo/VideoX-Fun"
            if _VFUN not in sys.path:
                sys.path.insert(0, _VFUN)
            from videox_fun.utils.fp8_optimization import (
                convert_model_weight_to_float8, convert_weight_dtype_wrapper,
            )
            print(f"[fp8] Quantizing FLF2V-14B DiT Linear weights to fp8_e4m3fn ...")
            t0 = _t.time()
            # Only quantize nn.Linear weights — they're matmul'd via .forward and the
            # autocast wrapper dequantizes them. Bare Parameters used in direct ops
            # (modulation, emb_pos, norm) MUST stay in bf16 because fp8 can't be
            # promoted in tensor arithmetic.
            n_linear = 0
            n_params = 0
            for mod in self.model.modules():
                if isinstance(mod, torch.nn.Linear) and mod.weight is not None:
                    ...
                    n_linear += 1
                    n_params += mod.weight.numel()
            # Wrap their forward to dequantize on the fly
            convert_weight_dtype_wrapper(self.model, dtype)
            torch.cuda.synchronize()
            print(f"[fp8] Quantized {n_linear} Linear layers ({n_params/1e9:.2f}B params) "
                  f"in {_t.time()-t0:.1f}s.  VRAM: {torch.cuda.memory_allocated()/1e9:.2f} GB")
            torch.cuda.synchronize()
            print(f"[fp8] Done in {_t.time()-t0:.1f}s.  "
                  f"VRAM after quant: {torch.cuda.memory_allocated()/1e9:.2f} GB")
