...
class WanWrapper:
    ...
    def load(self, device: str = "cuda", dtype: torch.dtype = torch.bfloat16,
        ...
        If `fp8=True`, the DiT weights are quantized to torch.float8_e4m3fn
        (memory savings ~2×) and dequantized on-the-fly to `dtype` for matmul
        via VideoX-Fun's wrapper.
        """
        ...
        if fp8:
            # Apply VideoX-Fun's fp8 weight quantization + autocast wrapper
            import sys as _sys
            _VFUN = "/home/rog/Desktop/gvcc_turbo/VideoX-Fun"
            if _VFUN not in _sys.path:
                _sys.path.insert(0, _VFUN)
            from videox_fun.utils.fp8_optimization import (
                convert_model_weight_to_float8, convert_weight_dtype_wrapper,
            )
            print(f"[fp8] Quantizing DiT weights to torch.float8_e4m3fn ...")
            t_fp8 = time.time()
            convert_model_weight_to_float8(self.model, exclude_module_name=['embed_tokens'])
            convert_weight_dtype_wrapper(self.model, dtype)
            torch.cuda.synchronize()
            print(f"[fp8] Done in {time.time()-t_fp8:.1f}s.")
